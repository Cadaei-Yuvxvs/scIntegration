...
#for(i in 1){
  ...
  param_set$categorical_covariates = c(param_set$batch_var) #character(0) # c("inferred_sex"), # need to pass batch var here !
  ...
  output_message = system(paste0("sbatch -J ",jobname," -o ",outputfile," -e ",errorfile," python/run_Python_slurm_GPU.sh ",singularity_path," ",script_path," ",param_file),intern = TRUE)
  #output_message = system(paste0("sbatch -J ",jobname," -o ",outputfile," -e ",errorfile," --dependency=afterok:",paste0(dependency_ids,collapse = ":")," --kill-on-invalid-dep=yes python/run_Python_slurm_GPU.sh ",singularity_path," ",script_path," ",param_file),intern = TRUE)
